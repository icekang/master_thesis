...
\subsubsection{Genesis}
Genesis provides its official code~\cite{Zhou2021}. Their implementation involves an initial pre-processing step on the Unannotated OCT images, sampling small cubes of size $64\times 64\times 64\times 32$ from the unannotated volumes. These cubes are then corrupted on the fly during the Genesis pre-training process. The author recommends $96$ cubes per volume, as increasing this number might not improve the performance while adding computation cost. We followed this recommendation and applied an additional sampling strategy. Our strategy randomly sampled $96$ cubes from each volume to ensure that the cubes contain enough information, as OCT images are filled with background unlike CT images used in the original paper. The sampling strategy involves selecting only the cubes with average intensities above a certain threshold. The resulting cubes from our thresholding strategy are illustrated in Figure~\ref{fig:genesis-cubes-with-threshold}. They contain information such as the lumen and wall of the artery. Without our thresholding strategy, the sampled cubes are filled with background noise, shown in Figure~\ref{fig:genesis-cubes-without-threshold}. We then trained the model on these cubes until convergence.
...
\begin{figure}[t]
    ...
    \begin{subfigure}{0.48\textwidth}
        ...
        \caption{Samples of cubes from unannotated OCT images with thresholding sampling strategy.}
    ...
    \begin{subfigure}{0.48\textwidth}
        ...
        \caption{Samples of cubes from unannotated OCT images without thresholding sampling strategy.}
...
For fine-tuning, we loaded the pre-trained weights of the model's encoder and decoder into the 3D nnUNet, leaving the segmentation head to be randomly initialized. Subsequently, we fine-tuned the pre-trained model on the Calcium OCT dataset. Following the findings of the 3D nnUNet, we fine-tuned the model with the best patch size found in Section~\ref{sec:design:nnunet}, which is $160\times 128\times 32$ of height by width by depth. To achieve this, we used the nnUNet's training script with the same configurations as the supervised pre-training and provided the pre-trained model as the initial weights.

\subsubsection{Multi-modal Self-Supervised Learning - CLIP}\label{sec:implementation:clip}

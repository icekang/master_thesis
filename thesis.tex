...
% This section is usually 5-10 pages.
We evaluated the effectiveness of self-supervised learning algorithms on calcium segmentation on coronary OCT images. All the models were fine-tuned on the Calcium OCT dataset using the validation set to select the best model. We did this for 3 folds. The models were then evaluated on the testing set. The performance was measured using the Dice score, a common metric for segmentation tasks, which is defined as follows:
...
The models are grouped into three categories: SSL for self-supervised learning models on our unannotated OCT, Transformer for supervised learning transformer-based models, and nnUNet for supervised learning nnUNet models. V-JEPA on unannotated OCT is categorized under SSL, whereas V-JEPA on VideoMix2M is under Transformer. Each result is compared statistically to the 2D nnUNet model, which has the highest average Dice score in our study. The results of the best models in each category are summarized in Figure~\ref{fig:results}.
...
In the SSL category, V-JEPA pre-trained on unannotated OCT images performs significantly worse than 2D nnUNet. As discussed in the previous section, the scale of the dataset may play an important role in transformer-based self-supervised learning. Genesis pre-training on unannotated OCT images also performs comparably to the 2D nnUNet. Additionally, it is as effective as supervised pre-training while not requiring additional annotations. CLIP pre-training on co-registered multi-modal OCT images also performs comparably to the 2D nnUNet model. Furthermore, it performs comparably to the 3D nnUNet model pre-trained on the LaW OCT dataset. This demonstrates that CLIP can be as effective as supervised pre-training while being less costly for annotation.
...
However, increasing the size of the dataset and model parameters may not provide further benefits for nnUNet beyond a certain point. In contrast, transformers are likely to benefit from scaling~\cite{Zhai2021}. This study does not aim to prove that nnUNet is superior to transformers in general. Instead, it demonstrates that nnUNet is more appropriate for calcium segmentation on OCT images given the current scale of our dataset and model parameters.

The best model in our study is 2D nnUNet, which achieves an average Dice score of $0.753\pm0.006$. However, continuity is better achieved and retained during the post-processing by 3D nnUNet than its 2D counterpart. 2D nnUNet, on the other hand, does not benefit from pre-training on other datasets, because converting the dataset into 2D images may provide sufficient samples for the model to learn the features of the calcium plaques. On the contrary, the best performance in the 3D nnUNet is achieved by pre-training on the LaW OCT dataset. Additionally, the patch size of 3D nnUNet is crucial for the model's performance. Reducing the patch size in the depth dimension significantly enhances the performance, whereas increasing context in the width and height dimension does not. This reflects the nature of annotating calcium plaques in OCT images. Calcium plaques are identified by changes in intensity around the artery walls. Therefore, an increment in the height and width does not provide additional information to the model. Similarly, calcium often occurs between adjacent slices, so the model doesn't need to have more context in the depth dimension.
...
One possible reason is that CLIP implicitly learns the features of the lumen and wall, while supervised pre-training explicitly learns those features. This leads to more data being needed for CLIP to translate the implicit features into the explicit features, as CLIP thrives as the scale of the dataset increases. 
...
Visually good features do not always lead to better performance, as the distinction of the most important features weighs more than their aesthetics. We visualize the features from V-JEPA (VideoMix2M) and the features from the ViT initial weights. Shown in Figure~\ref{fig:visually-good-features}, both retain the structure of the artery walls. However, the features from V-JEPA (VideoMix2M) put the background into the same color and provide more distinction to the artery walls. This may explain why V-JEPA (VideoMix2M) improves the model's performance. While the features from the initial weights of the model retain the original structure and the artery walls are visible to the human eye, the color of the features shows that the artery walls are not as distinguishable from the background or other structures. They blend into a gradient of colors. Features that appear good to human eyes may not be the best for the model to learn from.

...
\textit{TODO: Jigsaw, rotation, color, disnoising were used in CV traditionally, but in recent years, contrastive learning, reconstruction tasks are eliciting more attention.
SSL in Medical imaging is still in the traditional phase, using jigsaw, rotation, color, disnoising, patch shuffling, and deep clustering. However, recent researches have shown that contrastive learning, reconstruction tasks, and generative tasks are more effective in learning representations.
Thus, it is obvious that there is a gap worth exploring in SSL for medical imaging. Bridging the gap between traditional SSL and modern SSL in medical imaging can be beneficial for the field. Something something ...}

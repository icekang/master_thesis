...
A profuse amount of self-supervised learning researches on images are stemming from the success of its counterpart in natural language processing (NLP). In NLP, large language model is a groundbreaking invention that permits researchers to utilize unannotated data, which are omnipresent thanks to the advent of the internet. Researchers have realized that by masking a segment of texts, and letting the machine guess them by the surrounding texts, a context, the machine can be used for downstream natural language tasks thereafter. They call masking the texts and predicting them by their contexts a pre-text task, a task done before the main task, that does not require any human annotation. Along with texts on the internet come images. However, it is unclear what pre-text tasks could give as effective as the masked language modelling (MLM) in NLP. 

Visual self-supervised learning has been evolving from denoising, colorization, and solving jigsaw puzzle to aligning different views of the same image, restoring highly distorted parts of an image, and predicting the masked segments of an image by their contexts. Later, visual self-supervised learning has been extended to videos, as they have more contexts to learn from than static images. Additionally, videos and 3D images share similar characteristics, with videos incorporating a temporal dimension and 3D images adding spatial depth.

Coronary artery disease (CAD) is a primary cause of heart diseases, which is caused by plaques building up on the arteries, impeding the blood flow. Percutaneous coronary intervention (PCI) expands the arteries, but result of this treatment is affected coronary artery calcification (CAC), which interferes the expansion. Consequently, quantification of the calcium plaques is indispensable to the intravascular lithotripsy (IVL), which then sends sonic pressure waves to pulverize the plaques.

Acquiring human annotations for medical images is particularly challenging, because they need experts who understand specific medical image modalities. Specifically, the experts must identify the calcium deposits in the optical coherence tomography (OCT) images. Afterwards, the model replicate the experts by studying their segmentation masks. Therefore, they medical imaging can highly benefit from self-supervised learning, leveraging the unannotated OCT images to improve the segmentation.

There are gaps worth filling in the calcium segmentation on OCT images. First, the gap between the visual self-supervised learning on natural images and medical images. Recent visual SSL on natural images explores the efficacy of contrastive, and generative pre-text tasks, whereas, only contrastive tasks are well studied in medical imaging. The generative visual SSL on medical images are limited to restorative, leaving masked patch prediction to be investigated. Second, a way to capitalize the multi-modality nature of OCT images of patients treated for their CAD. In essence, the images of arteries before IVL, after IVL, and after stents are employed are captured. They are the same arteries at different stages of the treatment. It is worth studying what could they be used for after the image co-registration. Lastly, an alternative of visual self-supervised learning, a supervised pre-training. Supervised pre-training, or fine-tuning from the model trained for others tasks is a common practice in computer vision. In our setting, there are annotations for lumen and wall of the OCT images which we must examine along with other visual SSL.
...
We found that the best method to improve calcium segmentation is to pre-train the model to segment the lumen and wall of the arteries. However, this method requires possessing another set of medical image annotation relevant to the downstream task. Economical alternatives includes utilizing either multi-modal or restorative pre-training. While the former requires inexpensive co-registration, the latter does not need any annotation. 
This, of course, comes with trade-off which .% TODO: wati for analysis of the Genesis
Our experiments illustrate that masked patch prediction is not suitable as a pre-text task in medical imaging, leaving it for future research to explore further.

Our contributions are as follow:
\begin{enumerate}
    \item Comparing the CNN-based and transformer-based segmentation models.
    \item Studying self-supervised learning for medical imaging, including contrastive, restorative and generative approach, to beidge the gap between natural and medical visual self-supervised learning.
...
%     \caption{An example of an annotated image in the Calcium OCT dataset. The first row displays the original image in three axes, while the bottom row presents the segmentation mask for the corresponding views.}
%     \label{fig:calcium-oct}
% \end{figure}
...
% TODO: add a photo of the V-JEPA framework
...
% TODO: add a photo of the Genesis framework
